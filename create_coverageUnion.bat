...
set JAVA_HOME=java.exe
echo setting PATH
set PATH=java.exe;%PATH%"
...
for /L %%i in (1,1,%num%) do (
	if exist coverageunion\coverageunion.es (
		java -cp %emmapath% emma merge -in coverageunion\coverageunion_Thread-%%i_ciclo_%cycle%.es,coverageunion\coverageunion.es -out coverageunion\coverageunion.es
		REM echo esiste
	) else 	(
		java -cp %emmapath% emma merge -in coverageunion\coverageunion_Thread-%%i_ciclo_%cycle%.es -out coverageunion\coverageunion.es
		REM echo non esiste
	)
)
...
java -cp %emmapath% emma report -r txt -Dreport.txt.out.file=coverageunion\coverageunion.txt -in coverageunion\coverageunion.es
