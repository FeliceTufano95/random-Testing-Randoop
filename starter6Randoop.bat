...
REM nome del test
set name=%1
REM numero di dispositivi in parallelo 
rem set sessions=%2
REM numero di sessioni
set timelimit=%2
REM nome del thread
set thread=%3
REM valore del seme
...
java -noverify -cp %emmapath% emma instr -m fullcopy -d %mydatetime% -cp ".\jars\%name%.jar;.\jars\lib\*" -ix @myfilters.txt -out %mydatetime%\coverage.em
pause
...
for /L %%N in (1,1,%sequences%) do (
	...
	java -noverify -cp "%mydatetime%\lib\%name%.jar";%emmapath%;randoop-all-3.0.6.jar randoop.main.Main gentests --classlist=jars\%name%.txt --timelimit=%timelimit% --randomseed=!RANDOM! --no-regression-tests=true --junit-output-dir=ErrorTest_%thread%
	...
	if exist ErrorTest_%thread% (
		if !flag!==0 (
			xcopy saxpath ErrorTest_%thread%/E
	...
	java -cp %emmapath% emma report -r txt -Dreport.txt.out.file=%mydatetime%\coverage%cycle%-%thread%-%%N.txt -in %mydatetime%\coverage.em,%mydatetime%\coverage%cycle%-%thread%-%%N.ec
		

	REM fondi i file .em e .ec in un unico file .es
	java -cp %emmapath% emma merge -in %mydatetime%\coverage.em,%mydatetime%\coverage%cycle%-%thread%-%%N.ec -out %mydatetime%\coverage%cycle%-%thread%-%%N.es
	...
	if exist %mydatetime%\coverageunion%cycle%-%thread%.es 		(
		java -cp %emmapath% emma merge -in %mydatetime%\coverage%cycle%-%thread%-%%N.es,%mydatetime%\coverageunion%cycle%-%thread%.es -out %mydatetime%\coverageunion%cycle%-%thread%.es
	) else 		(
		java -cp %emmapath% emma merge -in %mydatetime%\coverage%cycle%-%thread%-%%N.es -out %mydatetime%\coverageunion%cycle%-%thread%.es
	)
...
if exist %mydatetime%\coverageunion%thread%.es (
	java -cp %emmapath% emma merge -in %mydatetime%\coverageunion%cycle%-%thread%.es,%mydatetime%\coverageunion%thread%.es -out %mydatetime%\coverageunion%thread%.es
	REM echo esiste
) else 		(
	java -cp %emmapath% emma merge -in %mydatetime%\coverageunion%cycle%-%thread%.es -out %mydatetime%\coverageunion%thread%.es
	REM echo non esiste
)
...
java -cp %emmapath% emma report -r txt -Dreport.txt.out.file=%mydatetime%\unioneSequenzeDellaSessione_%thread%_perCicliCrescenti.txt -in %mydatetime%\coverageunion%thread%.es
...
REM genera il report della copertura degli eventi
REM per output in HTML: -r html -Dreport.html.out.file=%mydatetime%\coverageunion\index.html
java -cp %emmapath% emma report -r txt -Dreport.txt.out.file=%mydatetime%\coverageunion%cycle%-%thread%.txt -in %mydatetime%\coverageunion%cycle%-%thread%.es


REM CREAZIONE DELL'UNIONE DI TUTTE LE SESSIONI IN QUESTO CASO PRODURRA' DUE FILE UGUALI
...
if exist %mydatetime%\coverageunion.es (
	java -cp %emmapath% emma merge -in %mydatetime%\coverageunion%cycle%-%thread%.es,%mydatetime%\coverageunion.es -out %mydatetime%\coverageunion.es
	REM echo esiste
) else 	(
	java -cp %emmapath% emma merge -in %mydatetime%\coverageunion%cycle%-%thread%.es -out %mydatetime%\coverageunion.es
	REM echo non esiste
)
...
REM per output in HTML: -r html -Dreport.html.out.file=%mydatetime%\coverageunion\index.html
java -cp %emmapath% emma report -r txt -Dreport.txt.out.file=%mydatetime%\coverageunion.txt -in %mydatetime%\coverageunion.es
