...
public class Run {
	...
	public static void main(String[] args) throws Exception {
		...
		num = input.nextInt();
		System.out.println("inserisci il nome dell'applicazione da testare (saxpath o jipa): ");
		...
		Consumatore c = new Consumatore(cc,num);
		var ex = new DrawLOC(num);
		var ex1 = new DrawErrorTest(num);
		
		for(int i=0; i<num; i++) {
			p[i] = new Produttore(cc, nomeApplicazione, timeLimit);
			p[i].start();
		}
		c.start();
